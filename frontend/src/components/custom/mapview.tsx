...
const MapView: React.FC<IMapViewProps> = ({ map, center }: IMapViewProps) => {
    ...
    return (
        <div ref={imgContainerRef} style={{
            ...
            <div ref={imgRef} className="max-w-none max-h-none" style={{ touchAction: "none", position: "absolute",left:position.x, top:position.y, transform:`scale(${scale})` }}>
            <img onLoad={()=>{setLoaded(true)}} className="relative w-auto h-full max-w-none max-h-none" src={map}  />
            <PinIcon style={{position:"absolute",left:center.x-40,top:center.y-50,color:"red",scale:'1'}} fontSize="large"/>
            </div>
        </div>
    );
}
